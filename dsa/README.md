# DSA Lab Programs

This directory contains C implementations of various Data Structures and Algorithms.

## 📂 Contents

1.  **Multiple Stacks**: Implementation of multiple stacks (`1_multiple_stacks.c`).
2.  **Circular Queue**: Circular queue operations (`2_circular_queue.c`).
3.  **Doubly Linked List**: DLL implementation (`3_doubly_linked_list.c`).
4.  **Binary Search Tree**: BST creation and operations (`4_bst.c`).
5.  **Tree Traversal**: Inorder, Preorder, Postorder traversals (`5_tree_traversal.c`).
6.  **Static Hashing**: Hashing techniques (`6_static_hashing.c`).

## 📚 Resources

- **GUIDE.md**: Detailed guide for the programs.
- **VIVA_QUESTIONS.md**: Common viva questions for these topics.
- **EDGE_CASES.md**: Edge cases to consider while testing.

## 🚀 How to Compile and Run

To compile a specific program:
```bash
gcc <filename>.c -o <output_name>
```

Example:
```bash
gcc 1_multiple_stacks.c -o stack
./stack
```
